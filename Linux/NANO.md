...
> [!TIP]
> The three most important Nano shortcuts to remember are:
>
> ```text
> Ctrl+O  → Save
> Ctrl+X  → Exit
...
> ```
>
> For quick terminal-based editing, these are enough to get started, while the rest of this reference provides the commands needed for navigation, selection, search, replacement and more advanced operations.
...
> [!IMPORTANT]
> Nano's keyboard bindings have changed between releases.
>
> GNU nano 8.0 introduced a new set of default "modern" bindings and also provides compatibility options for alternative bindings. The official Nano documentation and the shortcut menu displayed by your installed version should therefore be considered authoritative for exact key assignments.
>
...
> ```
>
> and view the current built-in shortcut reference with:
>
> ```text
> Ctrl+G
> ```
>
> You can also check command-line options with:
>
> ```bash
> nano --help
> ```
...
Този вариант следва същата конвенция като твоя Vim reference, но е съобразен с това, че **Nano е modeless editor** и че при Nano има важна разлика между текущите и старите keyboard bindings. Официалният сайт на GNU Nano в момента посочва 9.2 като latest версия и публикува отделно актуални cheatsheet/manual страници.

За останалите ти Linux reference-и бих запазил **точно тази структура и оформление**.